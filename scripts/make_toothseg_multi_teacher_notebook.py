...
cells = [
    md(
        "# CBCT ToothSeg teacher 2.5D multi-case student training",
        "",
        "Objective: use multiple ToothSeg/FDI-style teacher labelmaps to train a browser-sized",
        "2.5D YOLOv8 segmentation student that separates adjacent teeth better than post-processing alone.",
        "",
        "This notebook:",
        "",
        "1. Mounts Drive and locates paired CBCT image + ToothSeg label volumes.",
        "2. Exports physical neighbor/center/neighbor slices into RGB instance examples.",
    ...
    code(
        ...
        "EXP_NAME = 'toothseg-teacher-multicase-2p5d-1cls'",
        ...
        "# Browser inference is axial. RGB = z-2, z, z+2 after 0.3 mm resampling.",
        "TRAIN_AXES = ['z']",
        "CONTEXT_SLICES = 2",
        "STRIDE = 3",
        "MIN_AREA = 25",
    ...
    md(
        ...
        "",
        "The notebook first checks `clinic-raw/Dataset_clinic/imagesTr` + `labelsTr`,",
        "then searches extra teacher folders, then applies `MANUAL_PAIRS` overrides.",
    ),
    code(
        "MANUAL_PAIRS = [",
        "    {",
        ...
        "]",
        "DATASET_ROOTS = [",
        "    WORK / 'clinic-raw' / 'Dataset_clinic',",
        "]",
        "",
        ...
        "            continue",
        "        images = {}",
        "        for path in sorted(image_dir.glob('*.nii*')):",
        "            key = normalized_case_key(path)",
        "            images[key] = path",
        "        labels = {}",
        "        for path in sorted(label_dir.glob('*.nii*')):",
        "            key = normalized_case_key(path)",
        "            labels[key] = path",
        "        for key in sorted(set(images) & set(labels)):",
        "            found.append({'case': key, 'image': str(images[key]), 'labels': str(labels[key]), 'source': 'dataset'})",
        "    return found",
        "",
        "def discover_pairs():",
        "    images_by_key = {}",
        ...
        "",
        "pairs_by_case = {}",
        "for item in discover_dataset_pairs():",
    ...
    md("## 4. Make patient-level train, validation, and evaluation splits"),
    code(
        "import random",
        "split_pairs = pairs.copy()",
        "random.Random(SPLIT_SEED).shuffle(split_pairs)",
        "if USE_HELDOUT and len(split_pairs) >= 3:",
        "    holdout_n = max(1, round(len(pairs) * 0.2))",
        "else:",
        "    holdout_n = 0",
        "",
        "if holdout_n:",
        "    development_pairs = split_pairs[:-holdout_n]",
        "    eval_pairs = split_pairs[-holdout_n:]",
        "else:",
        "    development_pairs = split_pairs",
        "    eval_pairs = split_pairs",
        "",
        "if len(development_pairs) >= 2:",
        "    val_n = max(1, round(len(development_pairs) * 0.2))",
        "    train_pairs = development_pairs[:-val_n]",
        "    val_pairs = development_pairs[-val_n:]",
        "else:",
        "    train_pairs = development_pairs",
        "    val_pairs = []",
        "",
        "print('train cases:', [p['case'] for p in train_pairs])",
        "print('validation cases:', [p['case'] for p in val_pairs])",
        "print('eval cases:', [p['case'] for p in eval_pairs])",
        "if len(pairs) < 5 or not val_pairs or not holdout_n:",
        "    print('NOT PROMOTABLE: fewer than 5 cases or no independent validation/evaluation split.')",
    ),
    md("## 5. Export teacher labels into a YOLO instance dataset"),
    code(
        "shutil.rmtree(DATA, ignore_errors=True)",
        "case_summaries = []",
        "export_pairs = [(pair, 'train') for pair in train_pairs] + [(pair, 'val') for pair in val_pairs]",
        "if not val_pairs:",
        "    # Pipeline-only fallback. Adjacent slices leak across splits, so never use this for promotion.",
        "    export_pairs = [(pair, None) for pair in train_pairs]",
        "for pair, forced_split in export_pairs:",
        "    cmd = [",
        "        'python', str(EXPORTER),",
        "        '--volume', pair['image'],",
        "        '--labels', pair['labels'],",
        ...
        "        '--context-slices', str(CONTEXT_SLICES),",
        "        '--single-class',",
        "        '--label-mode', 'nonzero',",
        ...
        "    'contextMm': CONTEXT_SLICES * TARGET_SPACING,",
