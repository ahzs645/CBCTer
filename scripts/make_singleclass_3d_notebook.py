...
cells = [
    ...
    md("**Read:** ideal = ~17 separated teeth, recall near 17/17, few FPs ⇒ the **2D-detect → 3D-watershed**",
       "path works on your scanner; numbering-by-position (`fdiNumbering.ts`) is the last step. Over-split →",
       "raise `min_distance` (10–14); still-merged → lower it (5–6). Tune `CONF`/`MIN_VOX` too."),
