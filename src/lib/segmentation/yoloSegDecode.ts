...
export function buildUnionMask(
  ...
  for (const d of dets) {
    ...
    for (let y = y1; y < y2; y += 1) {
      ...
      const outRow = y * inputSize;
      for (let x = x1; x < x2; x += 1) {
        if (out[outRow + x]) continue;
...
export function decodeYoloSegMask(
  ...
  const mask = buildUnionMask(dets, out1, protoH, protoW, inputSize, options);
  return { mask, count: dets.length };
}
