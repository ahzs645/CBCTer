...
cells = [
    ...
    code("import math",
         ...
         "gt_items=[(v, centroid(lab==v)) for v in gt_vals]",
         "gc=np.array([c for _,c in gt_items]); gf=[v for v,_ in gt_items]",
         "gquad=np.array([v//10 for v in gf]); gpos=np.array([v%10 for v in gf])",
         "def axis_dir(mask): d=gc[mask].mean(0)-gc[~mask].mean(0); return d/(np.linalg.norm(d)+1e-9)",
         "UP=axis_dir(gquad<=2); RIGHT=axis_dir((gquad==1)|(gquad==4)); ANT=axis_dir(gpos<=2); ctr=gc.mean(0)",
         ...
         "print(f'{len(inst_masks)} teeth | FDI numbering accuracy {correct}/{tot} = {correct/max(1,tot):.2f}')",
         "for i,pv,tv,ok in rows[:40]: print(f'  inst{i:2d}: pred {pv}  true {tv}  {\"OK\" if ok else \"x\"}')"),
